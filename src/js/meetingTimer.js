...
export default class MeetingTimer {
  ...
  _beep() {
    this.soundIntervalHandler = window.setInterval(() => {
      ...
      if (
        !this.muted &&
        this.countAlarm &&
        this.alarmsPlayed++ >= this.countAlarm
      ) {
        this._reset();
      }
    }, 1000);
  ...
  _setLabelText(mins, secs) {
    ...
    this.circleWrapper.setAttribute(
      'data-secs',
      MeetingTimer._getTimeText(secs)
    );
  }
  ...
  _calcCircleOffset() {
    const offset =
      ((this.circleSeconds - this.totalSeconds) / this.circleSeconds) * 100;
  ...
  loadSettings(settings) {
    if (settings) {
      if (settings.hasOwnProperty('countAlarm')) {
        this.countAlarm = parseInt(settings['countAlarm'], 10) || 0;
      }
    }
  }
}
